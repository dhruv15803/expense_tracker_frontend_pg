  ...
  return (
    <>
      <div className="mx-10 my-4 text-blue-500">
        <button onClick={() => setIsAddExpense(!isAddExpense)}>{isAddExpense ? 'Cancel' : 'Add expense'}</button>
      </div>
      {isAddExpense &&       <div className="border-2 rounded-lg shadow-lg p-4 flex flex-col mx-10">
        <h1 className="text-xl text-blue-500 font-semibold my-2">
          Add expense category
        </h1>
        <form onSubmit={addExpenseCategory} className="flex items-center gap-2">
          <input
        ...
        <div>
          {addExpenseCategoryNameMsg.success ? (
            <p className="text-blue-500">{addExpenseCategoryNameMsg.message}</p>
          ) : (
            <p className="text-red-500">{addExpenseCategoryNameMsg.message}</p>
          )}
        </div>
        <h1 className="text-xl text-blue-500 font-semibold my-2">
          Add expense
